...
data_part="ext4";
if /tmp/busybox test -e /dev/lvpool/userdata ; then
    data_part=$(/tmp/busybox blkid /dev/mapper/lvpool-userdata | /tmp/busybox awk '{print $3}');
    data_part=$(echo "${data_part:6:4}");
fi

# first installation requires external sdcard
if ! /tmp/busybox test -e /dev/lvpool/swap || \
   [ "${data_part}" == "ext4" ] ; then
...
format_partitions() {
    ...
    /tmp/mkfs.f2fs /dev/lvpool/userdata;
}
...
# we're running on a mtd (current) device
    ...
    if /tmp/busybox test -e /dev/lvpool/system ; then
        if [ "$(/tmp/busybox blockdev --getsize64 /dev/mapper/lvpool-system)" != "${SYSTEM_SIZE}" ] || \
           [ "$(/tmp/busybox blockdev --getsize64 /dev/mapper/lvpool-swap)" != "${SWAP_SIZE}" ] || \
           [ "${data_part}" == "ext4" ] ; then
            warn_repartition;
            setup_lvm_partitions;
            format_partitions;
